...
class Trainer(BaseTrainer):
    ...
    def train_step(self, input_batch):
        ...
        has_smpl = input_batch[
            "has_smpl"
        ].byte()  # flag that indicates whether SMPL parameters are valid
        has_pose_3d = input_batch[
            "has_pose_3d"
        ].byte()  # flag that indicates whether 3D pose is valid
        ...
        if self.options.run_smplify:
            ...
            pred_pose = (
                rotation_matrix_to_angle_axis(pred_rotmat_hom)
                .contiguous()
                .view(batch_size, -1)
            )
        ...
        opt_keypoints_2d = opt_keypoints_2d / (self.options.img_res / 2.0)
