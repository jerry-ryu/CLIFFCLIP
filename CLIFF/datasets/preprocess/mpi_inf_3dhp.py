            ...
            for j, vid_i in enumerate(vid_list):
                ...
                if extract_img:
                    ...
                    vid_file = os.path.join(
                        seq_path, "imageSequence", "video_" + str(vid_i) + ".avi"
                    )
                ...
                for i, img_i in enumerate(img_list):
                    ...
                    scale = (
                        scaleFactor * max(bbox[2] - bbox[0], bbox[3] - bbox[1]) / 200
                    )
...
def test_data(dataset_path, out_path, joints_idx, scaleFactor):
    ...
    for user_i in user_list:
        seq_path = os.path.join(
            dataset_path, "mpi_inf_3dhp_test_set", "TS" + str(user_i)
        )
        # mat file with annotations
    ...
    np.savez(
        out_file, imgname=imgnames_, center=centers_, scale=scales_, part=parts_, S=Ss_
    )


def mpi_inf_3dhp_extract(
    dataset_path, openpose_path, out_path, mode, extract_img=False, static_fits=None
):
    scaleFactor = 1.2
