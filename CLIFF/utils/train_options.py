...
class TrainOptions:
    def __init__(self):
        ...
        train.add_argument(
            ...
            default=0,
            type=float,
            help="Weight of per-vertex loss",
        )
        train.add_argument(
            "--keypoint_loss_weight",
            default=5.0,
            type=float,
            help="Weight of 2D and 3D keypoint loss",
        )
        train.add_argument(
            "--pose_loss_weight",
            default=1.0,
            type=float,
            help="Weight of SMPL pose loss",
        )
        train.add_argument(
            "--beta_loss_weight",
            default=0.001,
            type=float,
            help="Weight of SMPL betas loss",
        )
        train.add_argument(
        ...
        train.add_argument(
            ...
            default=False,
            action="store_true",
            help="Run SMPLify during training",
        )
        train.add_argument(
            "--smplify_threshold",
            type=float,
        ...
        train.add_argument(
            ...
            default=100,
            type=int,
            help="Number of SMPLify iterations",
        )

        shuffle_train = train.add_mutually_exclusive_group()
        shuffle_train.add_argument(
            "--shuffle_train",
            dest="shuffle_train",
            action="store_true",
    ...
    def parse_args(self):
        ...
        if self.args.from_json is not None:
            ...
            self.args.log_dir = os.path.join(
                os.path.abspath(self.args.log_dir), self.args.name
            )
