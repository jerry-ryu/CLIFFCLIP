...
class PartRenderer:
    ...
    def __init__(self, focal_length=5000.0, render_res=224):
        ...
        self.faces = torch.from_numpy(
            SMPL(config.SMPL_MODEL_DIR).faces.astype(np.int32)
        ).cuda()
    ...
    def get_parts(self, parts, mask):
        ...
        parts_index = torch.floor(
            100 * parts.permute(0, 2, 3, 1).contiguous().view(-1, 3)
        ).long()
        parts = self.cube_parts[
            parts_index[:, 0], parts_index[:, 1], parts_index[:, 2], None
        ]
        parts *= mask
    ...
    def __call__(self, vertices, camera):
        ...
        cam_t = torch.stack(
            [
                camera[:, 1],
                camera[:, 2],
                2 * self.focal_length / (self.render_res * camera[:, 0] + 1e-9),
            ],
            dim=-1,
        )
        ...
        K[2, 2] = 1
        K[0, 2] = self.render_res / 2.0
        K[1, 2] = self.render_res / 2.0
